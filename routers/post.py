...
from models.models import dbUser, dbPost
...
def get_posts(
    ...
    posts = db.query(dbPost).order_by(dbPost.id).all()
    return posts
...
def new_post(
    ...
    newPost_Obj = dbPost(**newPost.model_dump())
    db.add(newPost_Obj)
    db.commit()
...
def full_update(
    ...
    query = db.query(dbPost).filter(dbPost.id == id)
    existing_post = query.first()
    if existing_post is not None:
        # updating data in DB

        query.update(
            cast(dict[Any, Any], post.model_dump()),
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="ID not FOUND! so can't Update!",
        )
